...
const fileToWrite: string = path.resolve(__dirname, config.mutationsDeleteFile)

function generateMuatationConstant(model: DMMF.Model) {
	const modelName = model.name
	const modelNameLower = modelName.charAt(0).toLowerCase() + modelName.slice(1)
	return `\nexport const ${modelNameLower}Delete = mutationField("${modelNameLower}Delete", {
				...
				resolve: async (_, args, ctx) => {
					...
					const result = await ctx.prisma.${modelNameLower}.delete({
						where
					});
					return result
				}
...
export async function generateDeleteMutations(models: DMMF.Model[]) {
	let inputs = ""
	let constants = ""
